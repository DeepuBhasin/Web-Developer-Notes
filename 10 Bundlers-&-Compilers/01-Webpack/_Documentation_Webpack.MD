...
* Webpack acts as a module bundler for JavaScript. Modules are individual pieces of code stored in separate files. To manage how these modules interact and execute within our application, we utilize import and export statements
...
* To check global package install in your system
```
npm ls -g --depth 0
```
...
7. Loaders
   1. CSS Loader
   2. SASS/SCSS Loader
   3. JS Loaders & Babel Loader
   4. File Loader (Loading Images)
   5. TypeScript Loader
...
```
**⚠️ Note :** Instead of multiple lines of import statements, you can create an **index.js or index.ts** file where you import all files. Then, you can import this index.js file to conveniently access all modules
...
node index.js
```
**⚠️ Note :** in **Node** No need to add ".js" because node.js understand automatically
...
**⚠️ Note :** we always have to create **index.js** file always
...
## 📘Setting Environment or Constant variables
...
4. **web.config.js** : Overall, this configuration tells Webpack to bundle your JavaScript application starting from index.js, outputting the result to bundle.js in the dist directory. It also includes a rule for processing CSS files using style-loader and css-loader.
...
**1. Loading CSS as normal import using mini-css-extract-plugin**
...
module.exports = {
    ...
    plugins: [
        new MiniCssExtractPlugin({
            // Extracting CSS code into Style File
            filename: 'styles.css',
        })
    ]
};
...
**2. Loading CSS as variable like Module concept using mini-css-extract-plugin**

```js
import style from "./componentFile.css"
```

1. **HelloWorldButton.module.css**
```css
.hello-world-button {
  font-size: 20px;
  padding: 7px 15px;
...
```js
// Loading CSS File as variable
import style from "./HelloWorldButton.module.css"
...
class HelloWorldButton {
    render() {
        ...
        // loading css from style variable
        button.classList.add(style['hello-world-button'])
        button.onclick = function () {
            const p = document.createElement('p');
            p.innerHTML = 'Hello World';

            // loading css from style variable
            p.classList.add(style['hello-world-text']);
            body.appendChild(p);
        }
        const body = document.querySelector('body');
        body.appendChild(button);
        ...
        rules: [
            {
                ...
                use: [
                    MiniCssExtractPlugin.loader,
                    {
                        // for loading css in js
                        loader: 'css-loader',
                        options: {
                            // for generating class names
                            modules: {
                                // generating class names for development mode
                                localIdentName: '[local]--[md4:hash:7]'

                                // generating class names for production mode
...
* We are loading Normal/Module scss or sass file just need to set rules only.

1. **HelloWorldButton.module.scss**
...
```js
// Loading CSS File
import style from "./HelloWorldButton.module.scss"
...
**A. Create Internal CSS using style tag with style-loader by using style-loader**
...
```
**B. Create External CSS files using mini-css-extract-plugin**

1. **Command to install package**

```
npm install sass-loader css-loader mini-css-extract-plugin --save-dev
```
...
![Image](./images/babel-loader-1.png)

![Image](./images/babel-loader-2.png)

**⚠️ Note :** you can include this bundled file without using **type="module"**
...
module.exports = {
    ...
    resolve: {
        //it will add the appropriate file extension at the end of the file based on what Webpack finds example ts or js file
        extensions: [".ts", ".js"]
    },
    module: {
        rules: [
...
1. Packages
```
npm i webpack webpack-cli typescript ts-loader
```
...
* It while in webpack it will create source code of only bundled files (weather its a single file or multiple files) while in TypeScript it will create source map of each ts-files which we are creating in folder.
1. Package Name
...
* We can achieve by using two ways
  * Without files (it will store every thing into memory)
  * With files, written below (it will not store every thing into memory)
