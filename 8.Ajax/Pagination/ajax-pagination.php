<?php
  ...
  $sql = "SELECT * FROM usertable LIMIT {$offset},{$limit_per_page}";
  ...
  if(mysqli_num_rows($result) > 0){
    ...
    // this code is used to make number of pages links 

    $sql_total = "SELECT count(id) as Totalcount FROM usertable";
    $records = mysqli_query($conn,$sql_total) or die("Query Unsuccessful.");
    $total_record = mysqli_fetch_assoc($records)['Totalcount'];
